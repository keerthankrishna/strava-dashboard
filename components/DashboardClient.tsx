...
import type { Run, RunType, DistanceFilter } from '@/lib/types'
...
export default function DashboardClient({ runs }: { runs: Run[] }) {
  ...
  const thisYearRuns = runs.filter(r => new Date(r.date).getFullYear() === CURRENT_YEAR)
  const longestRun = runs.reduce((best, r) => r.distanceKm > best.distanceKm ? r : best, runs[0])
  ...
  const filteredRuns = useMemo(() => {
    ...
    if (activeYear !== 'all') {
      filtered = filtered.filter(r => new Date(r.date).getFullYear() === Number(activeYear))
    }

    const range = DISTANCE_RANGES[activeDistance]
    if (range) {
      filtered = filtered.filter(r => r.distanceKm >= range[0] && r.distanceKm <= range[1])
    }

    return filtered
  }, [activeType, activeYear, activeDistance, runs])

  // Global stats for marquee
  const pb5k = runs
    .filter(r => Math.abs(r.distanceKm - 5) / 5 <= 0.1)
    .reduce<Run | null>((b, r) => !b || r.pacePerKm < b.pacePerKm ? r : b, null)

  const pb10k = runs
    .filter(r => Math.abs(r.distanceKm - 10) / 10 <= 0.1)
    .reduce<Run | null>((b, r) => !b || r.pacePerKm < b.pacePerKm ? r : b, null)

  const formatRunTime = (run: Run | null) => {
    if (!run) return 'N/A'
  ...
  const marqueeItems = [
    { label: 'KM TOTAL', value: formatNumber(Math.round(getTotalKm(runs))) },
    { label: 'RUNS', value: String(runs.length) },
    { label: 'AVG PACE', value: `${getAvgPace(runs)}/KM` },
    { label: '5K BEST', value: pb5k ? formatRunTime(pb5k) : 'N/A' },
    { label: '10K BEST', value: pb10k ? formatRunTime(pb10k) : 'N/A' },
    { label: 'M ELEVATION', value: formatNumber(getTotalElevation(runs)) },
    { label: 'AVG HR', value: `${getAvgHeartRate(runs)} BPM` },
    { label: 'KCAL TOTAL', value: formatNumber(getTotalCalories(runs)) },
  ]

  const pbs = getPersonalBests(runs)
  const monthlyData = getMonthlyVolume(runs)
  ...
  return (
    <main id="main-content">
      ...
      <HeroSection
        ...
        totalRunsThisYear={getTotalRunsThisYear(runs, CURRENT_YEAR)}
      ...
      <motion.div
        ...
        transition={{ duration: 0.5 }}
      >
        <YearInReview runs={runs} year={CURRENT_YEAR} />
      </motion.div>
